...
$where = "";
$params = [
    ':codigo_orden' => strtoupper($buscar)
];

if ($buscar !== '') {
    $where = "
        WHERE 
            u.codigo = :codigo_exacto
            OR u.codigo LIKE :buscar_codigo
            OR u.zona LIKE :buscar_zona
            OR u.estante LIKE :buscar_estante
    ";

    $params[':codigo_exacto'] = strtoupper($buscar);
    $params[':buscar_codigo'] = '%' . $buscar . '%';
    $params[':buscar_zona'] = '%' . $buscar . '%';
    $params[':buscar_estante'] = '%' . $buscar . '%';
}
...
$sql = "
    ...
    ORDER BY 
        CASE 
            WHEN u.codigo = :codigo_orden THEN 0 
            ELSE 1 
        END,
        u.estante ASC,
...
<body>
    ...
    <main class="container py-4">
        ...
        <section class="card search-card mb-4">
            <div class="card-body p-3 p-md-4">
                <form id="formBuscarUbicacion" method="GET" action="ubicacion.php" class="row g-3 align-items-end">
                    ...
                    <?php if ($buscar !== ''): ?>
                        <div class="col-6 col-md-3 col-lg-2">
                            <a href="ubicacion.php" id="btnLimpiarBusqueda" class="btn btn-outline-secondary w-100">
        ...
        <section class="card main-card" id="resultadoUbicaciones">
            <div class="card-header bg-white border-0 p-3 p-md-4">
                <div class="row align-items-center g-3">
                    <div class="col-md-8">
                        <h5 class="section-title mb-1">
                                ...
                                <tr>
                                    ...
                                    <td class="text-end">
                                        <div class="action-group justify-content-end">
                                            <a
                                                href="ubicacion.php?buscar=<?= urlencode($u['codigo']) ?>"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const formBuscar = document.getElementById('formBuscarUbicacion');
            const inputBuscar = document.getElementById('inputBuscarUbicacion');
            const resultadoUbicaciones = document.getElementById('resultadoUbicaciones');
            const btnLimpiar = document.getElementById('btnLimpiarBusqueda');

            if (!formBuscar || !inputBuscar || !resultadoUbicaciones) {
                return;
            }

            async function buscarUbicaciones(valor) {
                const url = 'ubicacion.php?buscar=' + encodeURIComponent(valor);

                resultadoUbicaciones.style.opacity = '0.55';
                ...
                try {
                    ...
                    const nuevoResultado = doc.getElementById('resultadoUbicaciones');

                    if (nuevoResultado) {
                        resultadoUbicaciones.innerHTML = nuevoResultado.innerHTML;
                    }

                    if (valor.trim() !== '') {
                        window.history.pushState({}, '', 'ubicacion.php?buscar=' + encodeURIComponent(valor));
                    } else {
                        window.history.pushState({}, '', 'ubicacion.php');
                    }
                    ...
                    resultadoUbicaciones.style.opacity = '1';
                }
            }
